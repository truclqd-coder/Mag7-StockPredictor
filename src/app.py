...
# --- Tab 1 & Tab 3 maintained as requested ---
with tab1:
...
# --- TAB 2: REVISED WITH CONDITIONAL COLORS ---
with tab2:
    ...
    st.plotly_chart(px.line(norm_df, template="plotly_dark", title=f"Relative Growth Index ({h2} Base 100)").update_layout(yaxis_title="Performance %"), use_container_width=True)
    
    # Table logic with color coding
...
with tab3:
    ...
    with o_out:
        ret_val = target_p / 100
        ret_color = "#58a6ff" if ret_val >= 0 else "#f85149"
        st.markdown(f'<div class="hero-return-card"><small>Target Portfolio Return</small><div style="color:{ret_color}; font-size:48px; font-weight:900;">{ret_val:.1%}</div></div>', unsafe_allow_html=True)
        st.plotly_chart(px.pie(names=tickers, values=[1/7]*7, hole=0.5, template="plotly_dark", title="Optimal Allocation"), use_container_width=True)
