...
with tab1:
    ...
    with col_l:
        ...
        fig1.update_layout(yaxis_title="Market Price (USD)", xaxis_title="Date", hovermode="x unified")
    ...
    with col_r:
        ...
        st.metric("1Y Target", f"${m['Target']:.2f}", delta=f"{upside:.1f}% Upside", help="Analyst Mean Target")
        st.metric("Beta (β)", f"{m['Beta']:.2f}", help="Market Sensitivity")
        st.metric("Ann. Volatility (σ)", f"{m['Volatility']:.1%}", help="Price Risk")

# --- TAB 2: Sector Comparison ---
with tab2:
    st.subheader("Relative Performance Benchmark")
    ...
    fig2 = px.line(norm_df, template="plotly_dark", title=f"Mag7 Sector Benchmarking (Normalized to 100)")
    fig2.update_layout(yaxis_title="Relative Return (Start = 100)", xaxis_title="Date", hovermode="x unified")
    fig2.add_hline(y=100, line_dash="dash", line_color="#8b949e")
    st.plotly_chart(fig2, use_container_width=True)
    
    # Summary Table
    df_sum = pd.DataFrame([{ 
        "Ticker": t, "Rating": all_meta[t]['Rating'], 
        "1Y Return": f"{((all_prices_5y[t].iloc[-1]/all_prices_5y[t].iloc[-252])-1):.1%}", 
        "P/E": all_meta[t]["PE"], "Beta": round(all_meta[t]['Beta'], 2) 
    } for t in tickers])
...
with tab3:
    if not OPTIMIZER_AVAILABLE:
        st.error("PyPortfolioOpt library not found. Please install it to use the Optimizer.")
    else:
        st.subheader("Mean-Variance Portfolio Allocation")
        o_in, o_out = st.columns([1, 2])
        ...
        with o_out:
            try:
                ...
                m2.metric("Portfolio Vol", f"{vol:.1%}")
                m3.metric("Sharpe Ratio", f"{sha:.2f}")
                
                final_w = {t: w for t, w in cleaned.items() if w > 0.01}
                fig_pie = px.pie(names=list(final_w.keys()), values=list(final_w.values()), hole=0.5, template="plotly_dark", title="Optimal Weights")
                st.plotly_chart(fig_pie, use_container_width=True)
            except:
                st.warning("Current parameters are infeasible. Lower the target return or increase expected views.")
