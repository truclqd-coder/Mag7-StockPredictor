...
# --- TAB 1: RESTORED ORIGINAL ANALYTICS ---
with tab1:
...
# --- TAB 2: RESTORED COMPARISON CHART ---
with tab2:
    ...
    summary_df = pd.DataFrame([{"Ticker": t, "1Y Return (%)": all_meta[t]['Return1Y'] * 100, "Rating": all_meta[t]['Rating'], "Beta": round(all_meta[t]['Beta'], 2)} for t in mag7])
    st.dataframe(summary_df.style.format({"1Y Return (%)": "{:.1f}%"}).applymap(color_ret, subset=['1Y Return (%)']), use_container_width=True)

# --- TAB 3: RESTORED FULL OPTIMIZER + GLOSSARY ---
with tab3:
    if OPTIMIZER_AVAILABLE:
        ...
        with o_in:
            ...
            user_views = {t: st.number_input(f"{t} Exp. Return %", value=float(mu_hist[t]*100), help="Adjust expected annual return.") for t in mag7}
        ...
        with o_out:
            try:
                ...
                st.markdown(f'<div class="hero-return-card"><small title="Weighted average return.">CALCULATED PORTFOLIO RETURN</small><div style="color:#58a6ff; font-size:48px; font-weight:900;">{ret:.1%}</div></div>', unsafe_allow_html=True)
                m1, m2 = st.columns(2)
                m1.metric("Risk (Volatility)", f"{vol:.1%}", help="Portfolio standard deviation."); m2.metric("Sharpe Ratio", f"{sha:.2f}", help="Excess return per unit of risk.")
